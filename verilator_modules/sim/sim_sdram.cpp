...
//void SimSDRAM::LoadData(std::string file, int skip) {
void SimSDRAM::LoadData(std::string file, unsigned to, unsigned size, unsigned skip) {
	...
	if (skip > 0) {
		fseek(ioctl_file, skip, 0);
	}

	if (size == 0)
		size = this->mem_size;

	if (this->mem_size < size + to) {
		console.AddLog("Sdram size small. Load failed.");
		return;
	}
	
	fread((this->mem + to), 1, size, ioctl_file);
}
